...
class TestSeededEndToEndPipeline:
    ...
    def test_fresh_workspace_run_now_generates_report_with_resolvable_sources(
        ...
        assert "_Generated by OpenCode_" in system_message["content"]
        assert "## Executive Brief" in system_message["content"]
        assert "## Top Highlights" not in system_message["content"]
        assert "## Source Details" not in system_message["content"]
    ...
    def test_fresh_workspace_second_run_is_idempotent(self, client, monkeypatch):
        ...
        report_ids = second_detail["links"]["reports"]
        assert len(report_ids) == 1
        report_id = report_ids[0]
        msg_resp = client.get(f"/api/report-threads/{report_id}/messages")
        assert msg_resp.status_code == 200
        system_messages = [m for m in msg_resp.json() if m["role"] == "system"]
        assert len(system_messages) == 1
        assert "_Generated by OpenCode_" in system_messages[0]["content"]
        assert "## Top Highlights" not in system_messages[0]["content"]
        assert "## Source Details" not in system_messages[0]["content"]
